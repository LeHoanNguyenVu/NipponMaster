...
import RememberDeviceModal from '../components/auth/RememberDeviceModal';
...
export default function Auth({ onAuthSuccess, onBack }: AuthProps) {
  ...
  const [pendingSocialData, setPendingSocialData] = useState<any | null>(null);
  const [showRememberModal, setShowRememberModal] = useState(false);
  
  // Zustand Store
  const { login, register, loginWithSocial, error, clearError, isLoading, isAuthenticated } = useAuthStore();
  ...
  // Listen for OAuth2 Callback or Popup message
  useEffect(() => {
    // 1. If this window is an OAuth popup callback returning to origin with hash tokens
    ...
    if (hash.includes('access_token=') || hash.includes('id_token=')) {
      ...
      if (window.opener && !window.opener.closed) {
        ...
        setPendingSocialData(payload);
        setShowRememberModal(true);
        window.location.hash = '#/auth';
      }
    }

    // 2. Listen for OAuth2 postMessage from popup
    const handleOAuthMessage = (event: MessageEvent) => {
      if (event.data && event.data.type === 'OAUTH_SUCCESS' && event.data.payload) {
        setPendingSocialData(event.data.payload);
        setShowRememberModal(true);
      }
    };
  ...
  const handleSocialLogin = (provider: 'GOOGLE' | 'FACEBOOK') => {
    ...
    const hasRealGoogleId = googleClientId && !googleClientId.includes('sample') && googleClientId.includes('.apps.googleusercontent.com');
    const hasRealFbId = fbAppId && !fbAppId.includes('sample') && fbAppId.length >= 15;

    if (provider === 'GOOGLE') {
      if (hasRealGoogleId) {
        // Mở Domain thật của Google accounts.google.com khi có Client ID thật
        const googleAuthUrl = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${googleClientId}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=token%20id_token&scope=openid%20email%20profile&nonce=${Date.now()}&prompt=select_account`;
        window.open(
          googleAuthUrl,
          'google_oauth_popup',
          `width=${width},height=${height},top=${top},left=${left},status=no,menubar=no,toolbar=no,location=yes`
        );
      } else {
        // Cửa sổ Popup tương tác chọn tài khoản Google chuẩn UX cho môi trường Dev
        const popupUrl = `${window.location.origin}${window.location.pathname}#/oauth/popup?provider=google`;
        window.open(
          popupUrl,
          'oauth_popup',
      ...
      if (hasRealFbId) {
        // Mở Domain thật của Facebook khi có App ID thật
        const fbAuthUrl = `https://www.facebook.com/v18.0/dialog/oauth?client_id=${fbAppId}&redirect_uri=${encodeURIComponent(redirectUri)}&scope=email,public_profile&response_type=token`;
        window.open(
          fbAuthUrl,
        ...
        // Cửa sổ Popup tương tác chọn tài khoản Facebook chuẩn UX cho môi trường Dev
        const popupUrl = `${window.location.origin}${window.location.pathname}#/oauth/popup?provider=facebook`;
        window.open(
          popupUrl,
          'oauth_popup',
          `width=${width},height=${height},top=${top},left=${left},status=no,menubar=no,toolbar=no,location=no`
  ...
  const handleConfirmRemember = async (rememberDevice: boolean) => {
    if (!pendingSocialData) return;
    try {
      await loginWithSocial(pendingSocialData, rememberDevice);
        ...
        <form className="space-y-6" onSubmit={handleLoginSubmit}>
          ...
          <div className="flex flex-col gap-3">
            ...
            <button
              ...
              disabled={isLoading}
              className="w-full flex items-center justify-center gap-3 py-2.5 px-4 rounded-xl border border-outline-variant/60 bg-surface-container-lowest hover:bg-surface-container-low transition-all duration-200 cursor-pointer group disabled:opacity-50 disabled:cursor-not-allowed"
            >
              ...
              <span className="text-sm font-semibold text-on-surface group-hover:text-primary transition-colors">Tiếp tục với Google</span>
            </button>
            ...
            >
              ...
              <span className="text-sm font-semibold text-on-surface group-hover:text-[#1877F2] transition-colors">Tiếp tục với Facebook</span>
            </button>
          </div>
  ...
  return (
    <div className="w-full min-h-screen">
      ...
      <RememberDeviceModal
        ...
        userEmail={pendingSocialData?.email}
        userName={pendingSocialData?.fullName}
        avatarUrl={pendingSocialData?.avatarUrl}
        provider={pendingSocialData?.provider}
        onConfirm={handleConfirmRemember}
      />
    </div>
  );
}
