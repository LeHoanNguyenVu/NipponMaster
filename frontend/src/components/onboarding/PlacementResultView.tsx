...
export default function PlacementResultView({ result, onConfirm, onRetry, isConfirming }: Props) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
      ...
      <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', alignItems: 'stretch' }}>
        ...
        <div style={{ flex: 1, minWidth: 200 }}>
          ...
          <div style={{
            ...
            <div>
              <div style={{ fontSize: 12, color: '#8c827b', marginBottom: 2 }}>Đề xuất đăng ký học</div>
      ...
      {result.questionReviews && result.questionReviews.length > 0 && (
        <div style={{ borderRadius: 18, border: '1.5px solid #e2dbce', overflow: 'hidden' }}>
          <div style={{ padding: '16px 22px', background: '#f7f5f0', borderBottom: '1px solid #e2dbce' }}>
            <h3 style={{ fontSize: 14, fontWeight: 700, color: '#5a5450', textTransform: 'uppercase', letterSpacing: '0.06em', margin: 0 }}>
              📋 Đáp Án Chi Tiết ({result.questionReviews.length} câu)
            </h3>
          </div>
          <div style={{ background: '#fff' }}>
            {result.questionReviews.map((review, idx) => {
              const isOpen = openIdx === idx;
              return (
                <div key={review.questionId} style={{ borderBottom: idx < result.questionReviews.length - 1 ? '1px solid #f1ede6' : 'none' }}>
                  {/* Accordion header */}
                  <button
